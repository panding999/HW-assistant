...
### 7. 质量评分主要依赖模型 `total_score`，可解释性还可以加强

涉及文件：
- `agent-python/app/agent_runtime.py`

现状：
- 模型返回 5 个维度分数和 `total_score`。
- 如果 `total_score > 0`，系统直接采用模型总分；否则使用本地加权兜底。

影响：
- 分数更贴近模型判断，但每次生成会有一定主观波动。
- 当前 UI 展示总分，维度分没有充分展示，用户不容易理解为什么没过。

建议：
- 保留模型总分，但在监控详情里展示五个维度。
- 对模型输出做一致性校验：如果 total_score 与五维加权差距过大，使用加权分或标记低置信。
- 面试中说“模型评分 + 规则兜底”，不要说是严格客观测量。
