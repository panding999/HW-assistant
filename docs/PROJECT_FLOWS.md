...
> 质量门控不是简单判断文本长度，而是模型审稿和本地规则结合。模型负责判断结构、证据、具体性和可提交程度，本地规则负责处理章节完整率和明显占位符，避免模型偶然误判。
...
- 设计并实现作业报告生成 Agent 闭环，覆盖资料解析、向量检索、任务类型识别、报告生成、质量审稿、自动改写和任务监控。
- 基于 ChromaDB 构建课程资料 RAG 检索链路，按作业维度隔离并重建 collection，支持 PDF/Markdown/TXT 资料切片、向量化和多路 Top-K 召回。
- 设计质量门控机制，引入结构完整度、证据贴合度、具体性、可提交性和风险五类审稿维度，并结合本地规则识别章节缺失和占位符。
- 实现自动改写版本保护：改写后重新评分，仅在改写质量不低于初稿时采用，避免低质量改写覆盖有效内容。
